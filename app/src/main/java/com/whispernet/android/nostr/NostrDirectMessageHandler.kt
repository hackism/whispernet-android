package com.whispernet.android.nostr
...
import com.whispernet.android.services.SeenMessageStore
import com.whispernet.android.ui.ChatState
import com.whispernet.android.ui.MeshDelegateHandler
import com.whispernet.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.whispernet.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("whispernet1:")) return@launch

                val base64Content = content.removePrefix("whispernet1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = WhispernetPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.whispernet.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch

                val noisePayload = com.whispernet.android.model.NoisePayload.decode(packet.payload) ?: return@launch
                val messageTimestamp = Date(giftWrap.createdAt * 1000L)
                val convKey = "nostr_${senderPubkey.take(16)}"
                repo.putNostrKeyMapping(convKey, senderPubkey)
                com.whispernet.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        payload: com.whispernet.android.model.NoisePayload,
        convKey: String,
        ...
        when (payload.type) {
            com.whispernet.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                val pm = com.whispernet.android.model.PrivateMessagePacket.decode(payload.data) ?: return
            ...
            com.whispernet.android.model.NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = com.whispernet.android.model.WhispernetFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.whispernet.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = WhispernetMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
