...
import com.whispernet.android.core.ui.utils.singleOrTripleClickable
...
fun TorStatusIcon(
    ...
    val torStatus by com.whispernet.android.net.TorManager.statusFlow.collectAsState()
    
    if (torStatus.mode != com.whispernet.android.net.TorMode.OFF) {
        val cableColor = when {
...
fun PeerCounter(
    ...
    isConnected: Boolean,
    selectedLocationChannel: com.whispernet.android.geohash.ChannelID?,
    ...
    Row(
        verticalAlignment = Alignment.CenterVertically,
        modifier = modifier.clickable { onClick() }.padding(end = 8.dp) // Added right margin to match "whispernet" logo spacing
    ) {
        Icon(
            imageVector = Icons.Default.Group,
            contentDescription = when (selectedLocationChannel) {
                is com.whispernet.android.geohash.ChannelID.Location -> stringResource(R.string.cd_geohash_participants)
...
private fun PrivateChatHeader(
    ...
    val isMutualFavorite = remember(peerID, peerNicknames) {
        try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                com.whispernet.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.isMutual == true
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.whispernet.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.isMutual == true
            } else false
    ...
    val titleText: String = if (isNostrDM) {
        ...
        val (conversationGeohash, baseName) = try {
            val repoField = com.whispernet.android.ui.GeohashViewModel::class.java.getDeclaredField("repo")
            repoField.isAccessible = true
            val repo = repoField.get(viewModel.geohashViewModel) as com.whispernet.android.nostr.GeohashRepository
            val gh = repo.getConversationGeohash(peerID) ?: "geohash"
            val fullPubkey = com.whispernet.android.nostr.GeohashAliasRegistry.get(peerID) ?: ""
        ...
        peerNicknames[peerID] ?: run {
            val titleFromFavorites = try {
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    val noiseKeyBytes = peerID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                    com.whispernet.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.peerNickname
                } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    com.whispernet.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.peerNickname
                } else null
...
private fun MainHeader(
    ...
    val bookmarksStore = remember { com.whispernet.android.geohash.GeohashBookmarksStore.getInstance(context) }
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 14.dp)) {
                ...
                val currentGeohash: String? = when (val sc = selectedLocationChannel) {
                    is com.whispernet.android.geohash.ChannelID.Location -> sc.channel.geohash
                    else -> null
                }
...
private fun LocationChannelsButton(
    ...
    val (badgeText, badgeColor) = when (selectedChannel) {
        ...
        is com.whispernet.android.geohash.ChannelID.Location -> {
            val geohash = (selectedChannel as com.whispernet.android.geohash.ChannelID.Location).channel.geohash
