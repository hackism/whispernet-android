...
object NotificationTextUtils {
     ...
     */
    fun buildPrivateMessagePreview(message: WhispernetMessage): String {
        return try {
            when (message.type) {
                WhispernetMessageType.Image -> "📷 sent an image"
                WhispernetMessageType.Audio -> "🎤 sent a voice message"
                WhispernetMessageType.File -> {
