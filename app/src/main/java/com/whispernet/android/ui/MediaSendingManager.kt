...
import com.whispernet.android.model.WhispernetFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.whispernet.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = WhispernetFilePacket(
                fileName = file.name,
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, WhispernetMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, WhispernetMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, WhispernetMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, WhispernetMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.whispernet.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> WhispernetMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> WhispernetMessageType.Audio
                else -> WhispernetMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: WhispernetFilePacket,
        filePath: String,
        messageType: WhispernetMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.whispernet.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.whispernet.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.whispernet.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.whispernet.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
