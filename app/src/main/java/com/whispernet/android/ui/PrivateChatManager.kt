...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<WhispernetMessage>>()
    ...
    fun startPrivateChat(peerID: String, meshService: BluetoothMeshService): Boolean {
        if (isPeerBlocked(peerID)) {
            ...
            val systemMessage = WhispernetMessage(
                sender = "system",
    ...
    fun handleIncomingPrivateMessage(message: WhispernetMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: WhispernetMessage, suppressUnread: Boolean) {
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.whispernet.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
