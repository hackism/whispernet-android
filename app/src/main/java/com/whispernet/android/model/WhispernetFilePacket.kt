...
/**
 * WhispernetFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 *
 * Note: The outer WhispernetPacket uses version 2 (4-byte payload length), so this
 ...
 */
data class WhispernetFilePacket(
    val fileName: String,
    val fileSize: Long,
    val mimeType: String,
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("WhispernetFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("WhispernetFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("WhispernetFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("WhispernetFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("WhispernetFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("WhispernetFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): WhispernetFilePacket? {
            android.util.Log.d("WhispernetFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                val result = WhispernetFilePacket(n, s, m, c)
                android.util.Log.d("WhispernetFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("WhispernetFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
