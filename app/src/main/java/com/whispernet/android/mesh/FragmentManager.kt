...
import com.whispernet.android.model.FragmentPayload
...
class FragmentManager {
    ...
    companion object {
        ...
        private const val FRAGMENT_SIZE_THRESHOLD = com.whispernet.android.util.AppConstants.Fragmentation.FRAGMENT_SIZE_THRESHOLD // Matches iOS: if data.count > 512
        private const val MAX_FRAGMENT_SIZE = com.whispernet.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_SIZE        // Matches iOS: maxFragmentSize = 469 
        private const val FRAGMENT_TIMEOUT = com.whispernet.android.util.AppConstants.Fragmentation.FRAGMENT_TIMEOUT_MS     // Matches iOS: 30 seconds cleanup
        private const val CLEANUP_INTERVAL = com.whispernet.android.util.AppConstants.Fragmentation.CLEANUP_INTERVAL_MS     // 10 seconds cleanup check
    }
     ...
     */
    fun createFragments(packet: WhispernetPacket): List<WhispernetPacket> {
        try {
        ...
        val fragments = mutableListOf<WhispernetPacket>()
        ...
        for (index in fragmentChunks.indices) {
            ...
            val fragmentPacket = WhispernetPacket(
     ...
     */
    fun handleFragment(packet: WhispernetPacket): WhispernetPacket? {
        ...
        try {
            ...
            if (fragmentMap != null && fragmentMap.size == fragmentPayload.total) {
                ...
                val originalPacket = WhispernetPacket.fromBinaryData(reassembledData.toByteArray())
...
interface FragmentManagerDelegate {
    fun onPacketReassembled(packet: WhispernetPacket)
}
