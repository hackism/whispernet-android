package com.whispernet.android.protocol
...
data class WhispernetPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = WhispernetPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            ttl = com.whispernet.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): WhispernetPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: WhispernetPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): WhispernetPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): WhispernetPacket? {
        try {
