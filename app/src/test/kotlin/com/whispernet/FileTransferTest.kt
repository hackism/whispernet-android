...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = WhispernetFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = WhispernetFilePacket.decode(encoded!!)
    ...
    fun `encode file packet with filename should include filename TLV`() {
        ...
        val packet = WhispernetFilePacket(
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = WhispernetMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.whispernet.android/files/images/photo.jpg",
            type = WhispernetMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = WhispernetMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.whispernet.android/files/audio/voice.amr",
            type = WhispernetMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = WhispernetMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.whispernet.android/files/documents/document.pdf",
            type = WhispernetMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.whispernet.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
